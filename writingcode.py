# Introduction
# We have learned how to create flowcharts for if conditions. 
# But we cannot use these flowcharts unless we learn how to write a code by looking at a flowchart.
...
# In the above example, we take the day as input from the user. 
# According to the day enetered, we print the food item for that particular day. 
# Read the comments carefully to understand how the code is executed at every step.

# Note: While entering the input for the day, you'll have to enter it exactly 
# as it is used in the if conditions. 
# Since Python is case-sensitive, it will treat Monday and monday differently.

# More examples
# We have only used == comparison operator in the if statements. 
# In the booleans exercise, we had seen other comparison operators as well. 
# For example:
# >: Greater than
# <: Less than
...
# Example 2
# Let's assume a rule that if the speed of our vehicle is less than or equal to 60, 
# it will not be considered as overspeeding. 
# Otherwise, it will be considered as overspeeding. We can write the program for it as shown below:
...
# Note: Notice we have converted the value in the speed variable to an int before using in if condition. 
# This is because the output of the input function is always a string 
# and we cannot compare a string with a number correctly using <= operator.
...
# For example on monday, "Poha" in breakfast, "Rajma Chawal" for lunch and "Roti Sabzi" for dinner. 
# Similary on tuesday, "Poori Sabzi" for breakfast, "Thukpa" for lunch and "Chicken Chawal" for dinner. 
# We can write a program for this question as shown below:
...
# Observe how we have used if statement inside another if statement. 
# The if statement inside will only be checked if the outer if condition is True.